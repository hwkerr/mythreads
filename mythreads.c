...
// requires/ensures interrupts are Enabled
...
// requires/ensures interrupts are Enabled
// requires/ensures the current thread has control of the lock
...
// requires/ensures interrupts are Disabled
...
// sets values of Node that should not persist after the Node's thread exits
// frees the stack pointer of the Node unless Node represents main context
void exitNode(int id)
{
  struct Node* node = getNode(id);

  if (node->status != EXITED && id != 0) // Don't try to free the main context
...
// Print the list of thread Nodes in order of priority
void printList()
{
  struct Node *node = head;
  while (node != NULL)
